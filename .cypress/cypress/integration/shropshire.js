...
it('does not allow typing alphabetic characters in the phone field', function() {
    ...
    cy.get('input.govuk-radios__input[value="Flytipping"]').click({force:true});
    cy.get('button.btn--final:contains(Continue)').first().click({force:true});
    // photos page
    cy.get('button.btn--block:contains(Continue)').first().click({force:true});
