...
FixMyStreet::override_config {
    ...
    $opts->@{qw(show_emails reports)} = undef;
    ...
    $opts->{show_emails} = 1;
    $opts->@{qw(commit show-emails)} = (0, 0);

    subtest 'using --show_emails does not change report state ...' => sub {
        $archive_reports->($opts);
    };

    subtest '... but does output demo emails' => sub {
    ...
    subtest 'using --show-emails (hyphen) also does not change report state but does output demo emails' => sub {
        $archive_reports->($opts);
        stdout_like { $archive_reports->($opts) } 
            qr/As part of this process we are closing all reports made before the update./,
            "closure emails output to STDOUT";
